...
[source,java]
----
include::{examples-dir}/consumer/ConsumerTest.java[]
----

<1> Add necessary annotations to the class. This defines the provider mock as well as the target directory for the generated pact file.
...
<7> We have hardcoded the mock response, so the colour parameter we pass in is irrelevant (for this particular test).
It tests how the consumer transforms the data that the mock passes back, when the consumer calls it, *not* the mock itself. The mock needs to get 
called via a rest call from inside the knit method.
